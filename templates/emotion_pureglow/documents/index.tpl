...
{foreach from=$Pages item=postions name="pagingLoop" key=page}
	...
	<div id="head_bottom" style="clear:both">
		{block name="document_index_head_bottom"}
			...
			{s name="DocumentIndexPageCounter"}Seite {$page+1}asdads von {$Pages|@count}{/s}
		 {/block}
	</div>
	...
	<tr>
		...
		{block name="document_index_table_name"}
			...
			{else}
				{s name="DocumentIndexPositionNameDefault"}hier drin {$position.supplierName|nl2br}  {$position.name|nl2br}{/s}
			{/if}
			</td>
		{/block}
