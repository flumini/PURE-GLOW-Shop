...
<div class="artbox{if $lastitem} last{/if}{if $firstitem} first{/if}">
	<div class="inner">
		...
		{else}
			{assign var=image value=$sArticle.image.src.11}
		{/if}
		<a href="{$sArticle.linkDetails|rewrite:$sArticle.articleName}" class="" title="{$sArticle.articleName}"><img src="{$articleimg1}" onmouseover="this.src='{$articleimg2}'"
        onmouseout="this.src='{$articleimg1}'" style="align: center;">
		...
		<br/><a href="{$sArticle.linkDetails|rewrite:$sArticle.articleName}" class="" title="{$sArticle.articleName}">{$sArticle.articleName|truncate:47}</a></p>
				{/block}
