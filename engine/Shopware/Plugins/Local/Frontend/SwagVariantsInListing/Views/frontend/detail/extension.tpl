				...
				{block name='frontend_detail_index_name'}
					...
					{if $sArticle.supplierName}
						{block name='frontend_detail_index_supplier'}
							<p class="supplier"><strong>{$sArticle.supplierLogo}</strong></p>
						{/block}
			    	{/if}
					<h1>{if $sArticle.pseudoprice}<img src={link file='frontend/_resources/images/icons/ico_sale_detail.png'} style="vertical-align: middle;
margin-right: 5px;">{/if}{$sArticle.articleName}</h1>
				{/block}
							...
							{block name="frontend_detail_index_inner_tabs"}
								...
								{block name="frontend_detail_index_tabs_description"}
									{include file="frontend/detail/description.tpl"}
																	...
																	{* Links *}
	{if $sArticle.sLinks && !empty($morearticles)}

		<div class="morearticles">
		<h2>{se name="DetailDescriptionLinkInformation"}{/se} {$sArticle.supplierName}</h2>
		<ul>
                {foreach from=$morearticles item=product}
                <li class=""><a rel="nofollow" class="article_image" href="{$product.link}">
                <img src="{$product.images}" onmouseover="this.src='{$product.images1}'"
        onmouseout="this.src='{$product.images}'" style="align: center;">
				</a><br/>
                <a rel="nofollow" class="article_description" title="{$product.title}" href="{$product.link}"><strong>{$product.title}</strong></a></li>
...
{/if}
                            	...
                            	{block name="frontend_detail_index_tabs_related"}
                               	 {include file="frontend/detail/related_neu.tpl"}
                            	{/block}
