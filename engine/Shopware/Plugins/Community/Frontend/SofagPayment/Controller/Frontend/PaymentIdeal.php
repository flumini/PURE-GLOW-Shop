<?php
    ...
    class Shopware_Controllers_Frontend_PaymentIdeal
        extends Shopware_Plugins_Frontend_SofagPayment_Controller_Frontend_PaymentAbstract
    {
        ...
        public function gatewayAction()
        {
            ...
            $senderBankCode = $model->getAttribute()->getSofagIdealBank();
            if ( $senderBankCode === null ) {
                $senderBankCode = Shopware()->Session()->sofagIdealBankCode;
        ...
        public function returnAction()
        {
            ...
            $tempTransactionId = Shopware()->Session()->sofagUniqueId;
            $ordernumber       = $helper->database()->getOrdernumberByTransactionId( $tempTransactionId );
            ...
            Shopware()->Session()->sofagTransactionId = $transactionId;
            $helper->database()->changeTransactionId( $ordernumber, $transactionId );
            $logger->logManually( $this->_loggingSource,
                "Changed Transaction id of order $ordernumber from $tempTransactionId to $transactionId" );
            //Change order state
            $order = Shopware()->Modules()->Order();
            if ( $state == 0 ) {
                $state = $helper->database()->getOderStateById( $orderId );
            }
            $order->setPaymentStatus( $orderId, $state, true );
            $logger->logManually( "Ideal Notification", "Changing state of order $ordernumber to $state." );

            Shopware()->Session()->sofagSendMail = true;
            $user = $this->getUser();
            $order->sUserData = $user;
            $order->sNet = empty($user['additional']['charge_vat']);
            $order->sendMail(Shopware()->Session()->sofagMailVariables);
