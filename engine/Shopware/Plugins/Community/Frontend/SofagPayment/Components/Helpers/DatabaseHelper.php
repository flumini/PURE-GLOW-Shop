 ...
 */
class Shopware_Plugins_Frontend_SofagPayment_Components_Helpers_DatabaseHelper
{
    /**
    ...
    {
        try {
            $sql = "CREATE TABLE IF NOT EXISTS `sofag_payment_basket` (" .
                   "`id` int(11) NOT NULL," .
            ...
            Shopware()->Log()->Err("There was an Error creating the Basket-Table: " . $exception->getMessage());
    ...
    {
        try {
            $sql = "ALTER TABLE sofag_payment_basket DROP PRIMARY KEY, ADD PRIMARY KEY(id);";
            Shopware()->Db()->query($sql);
        } catch (Exception $exception) {
            Shopware()->Log()->Err("There was an Error updating the Sofort Basket-Table: " . $exception->getMessage());
            throw new Exception("There was an Error creating the Sofort Basket-Table: " . $exception->getMessage());
        }
    }

    /**
     * Removes the sofag_payment_basket table from the db
     *
     * @throws Exception
     */
    public function removeBasketTable()
    {
        $sql = "DROP TABLE IF EXISTS `sofag_payment_basket`";
        try {
            Shopware()->Db()->query($sql);
        } catch (Exception $exception) {
            Shopware()->Log()->Err("There was an Error removing the Basket-Table: " . $exception->getMessage());
    ...
    {
        $sql = "REPLACE INTO `sofag_payment_basket` SELECT * FROM `s_order_basket` WHERE `sessionID`=?;";
        try {
            Shopware()->Db()->query($sql, array($sessionId));
            Shopware()->Db()->query("DELETE FROM `sofag_payment_basket` WHERE  DATEDIFF(NOW(), `datum`) >= 14;");
        } catch (Exception $exc) {
            echo $exc->getMessage();
        }
    }
    ...
    {
        $sql = "INSERT INTO `s_order_basket` SELECT * FROM `sofag_payment_basket` WHERE `sessionID`=?;"
               . "DELETE FROM `sofag_payment_basket` WHERE `sessionID`=?;";
        Shopware()->Db()->query($sql, array($sessionId, $sessionId));
    }
    ...
    public function changeTransactionId($ordernumber, $transactionId)
    {
        if (empty($ordernumber) || empty($transactionId)) {
            throw new Exception("Trying to change a transaction id using illegal parameters");
        }
    ...
    public function removeSnippets()
    {
        ...
        try {
            ...
            Shopware()->Log()->Err("There was an Error removing the snippets: " . $exception->getMessage());
    ...
    {
        try {
            $sql = "CREATE TABLE IF NOT EXISTS `sofag_payment_log` (" .
    ...
    /**
     * Removes the sofag_payment_log table from the db
     *
     * @throws Exception
     */
    public function removeLogTable()
    {
        $sql = "DROP TABLE IF EXISTS `sofag_payment_log`";
        try {
            Shopware()->Db()->query($sql);
        } catch (Exception $exception) {
            Shopware()->Log()->Err("There was an Error removing the Log-Table: " . $exception->getMessage());
    ...
    public function getLog($start, $limit)
    {
        ...
        $read = "SELECT * FROM `sofag_payment_log` LIMIT " . $start . ", " . $limit;
    ...
    /**
     * Inserts an entry in the log table (sofag_payment_log)
     *
    ...
    {
        $sql = "INSERT INTO `sofag_payment_log`(`version_module`,`source`, `message`)
            VALUES(?,?,?)";
    ...
    {
        $getTotal = "SELECT count(*) FROM `sofag_payment_log`";
    ...
    public function removePaymentRules($paymentId)
    {
        ...
        try {
            ...
            Shopware()->Log()->Err("There was an Error removing the payment rules: " . $exception->getMessage());
