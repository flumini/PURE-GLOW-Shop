...
{block name='frontend_index_header_javascript_inline' prepend}
    ...
    var cookiePermissionUrl = '{url module=widgets controller=SwagCookiePermission action=isAffectedUser forceSecure}';
{/block}
...
{block name='frontend_index_before_page' prepend}
    <div class="cookie-bar"
        ...
        <p>{s name='cookieBarMessage'}This page requires cookies. Do you agree with the usage of cookies?{/s}
            <a href="javascript: void(0);" class="cp-enable cp-btn">
                <span class="cp-yes-icon cp-icon"></span><span class="cp-btn-label">{s name='CookieBarYesBtn'}Yes{/s}</span></a>
            <a href="javascript: void(0);" class="cp-disable  cp-btn">
                <span class="cp-no-icon cp-icon"></span><span class="cp-btn-label">{s name='CookieBarNoBtn'}No{/s}</span>
            </a>
        </p>
    </div>
{/block}