...
{block name="frontend_index_header_javascript" append}
	<script type="text/javascript">
		...
		$(document).ready(function () {
			$("#userPostOffice").on('submit', function (e) {
				e.preventDefault();

				var postOfficeId = document.getElementById('pbId').value,
					zip = document.getElementById('zip').value,
					city = document.getElementById('city2').value,
					street = document.getElementById('streetName').value,
					streetNumber = document.getElementById('streetNumber').value;

				window.parent.updatePostOfficeShipping(postOfficeId, zip, city, street, streetNumber);
	...
	<div style="padding-left: 10px">
		<form name="frmPost" method="POST" action="{url controller=dhl action=getPostoffices sTarget=$sTarget}">
			{*zip code*}
	...
	<div style="padding-left: 10px">
		<form id="userPostOffice" name="userPostOffice" method="POST"
			  action="{url controller=dhl action=selectShipping sTarget=$sTarget pb=true }">
			{*post office ID*}
			<input type="text" id="pbId" name="pbId" size="10">
			{*streetName*}
	...
	<table>
		{*sidepanel to display the address of all the post offices*}
		<tr>
			{if $postOffices|is_array}
				<td id="leftColumn">
					<div id="leftContent">
						...
						{if isset ($postOffices)}
							{foreach $postOffices as $key => $postOffice}
								{$postOfficeId = $postOffice->depotServiceNo}
								...
								{if isset($postOfficeId)}
									<div class="content">
										...
										<div id="locationAddress">
											...
											<div>{s namespace="frontend/account/iFrameSearch" name='postOffice'}Postfiliale{/s}
												<span class="id">{$postOfficeId}</span>
											</div
											<div>
	...
	{if $mapKey}
		<script type="text/javascript">
			...
			function showLocation() {
				...
				postOfficeIdArray.push({$postOfficeId});
				...
				for (i = 0; i < latArray.length; i++) {
					...
					marker.set('postOfficeId', postOfficeIdArray[i]);
					...
					google.maps.event.addListener(marker, 'click', (function (marker, i) {
						return function () {
							var postOfficeId = this.get('postOfficeId');
							...
							document.getElementById('pbId').value = postOfficeId;
